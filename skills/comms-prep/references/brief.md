...
Cover the following six areas in a short brief, together or separately. Make material unknowns
explicit, with a way forward: check a source, make a supported assumption, ask, or limit the claim.
Use the questions to resolve the communication's needs; they are not a required output template.
...
5. **Form factor.** What's the right form factor for this communication? Short message, checklist,
   ...
   audiences. For long, multi-document, or structurally complex communications, read
   [Structure planning](structure.md) before drafting prose. If communication *about* this
   communication is required — for example, surfacing issues, uncertainties, or decisions — plan where it will
...
Record the review depth and reason using the entrypoint's review decision.
